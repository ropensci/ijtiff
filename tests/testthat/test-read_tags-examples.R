test_that("`read_tags()` works", {
  ...
  expect_equal(read_tags(path, "all")[c(2, 4)],
               read_tags(path, frames = c(2, 4)))
  expect_error(
    read_tags(path, frames = c(11, 12)),
    paste(" You have requested frame number 12 but there are",
  ...
  expect_equal(
    read_tags(path),
    list(
      frame1 = list(width = 6L, length = 15L, bits_per_sample = 8L,
                    samples_per_pixel = 1L, sample_format = "uint",
                    planar_config = "contiguous",
                    rows_per_strip = 15L, compression = "none",
                    description = paste0("ImageJ=1.51s\nimages=10\n",
                                         "channels=2\nframes=5\n",
                                         "hyperstack=true\nmode=composite\n",
                                         "loop=false\n"),
