...
SEXP write_tif_C(SEXP image, SEXP where, SEXP sBPS, SEXP sCompr, SEXP sFloats,
                SEXP sDesc, SEXP sRes, SEXP sResUnit, SEXP sColorSpace) {
  check_type_sizes();
  SEXP dims, img_list = R_NilValue;
  tiff_job_t rj;
  TIFF *tiff;
  FILE *f;
  ...
  float xres = 0, yres = 0;
  uint16_t res_unit = 2; // Default to inches
  int color_space = asInteger(sColorSpace);
  
  if (sDesc != R_NilValue) {
    if (TYPEOF(sDesc) != STRSXP || LENGTH(sDesc) == 0) {
      Rf_error("description must be a character string");
    }
    description = CHAR(STRING_ELT(sDesc, 0));
  }
  
  if (sRes != R_NilValue) {
    if (TYPEOF(sRes) != REALSXP || LENGTH(sRes) != 2) {
      Rf_error("resolution must be a numeric vector of length 2");
    }
    xres = (float)REAL(sRes)[0];
    yres = (float)REAL(sRes)[1];
  }
  
  if (sResUnit != R_NilValue) {
    res_unit = (uint16_t)asInteger(sResUnit);
  }
  ...
  while (true) {
    if (img_list != R_NilValue) image = VECTOR_ELT(img_list, img_index++);
    ...
    if (sRes != R_NilValue) {
      TIFFSetField(tiff, TIFFTAG_RESOLUTIONUNIT, res_unit);
    ...
    _TIFFfree(buf);
    if (img_list != R_NilValue && img_index < n_img) {
